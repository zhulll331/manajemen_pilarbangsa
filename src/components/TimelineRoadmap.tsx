    ...
    <section className="bg-white border border-gray-100 rounded-3xl p-6 sm:p-12 shadow-xl my-16 space-y-12 relative overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="relative pt-8">
          ...
          {/* 1. MODE DESKTOP: Horizontal Timeline (hidden on mobile, block on lg) */}
          <div className="hidden lg:block overflow-x-auto pb-12 pt-4 px-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
              ...
              {filteredEvents.map((event, index) => {
                ...
                return (
                  <div key={event.id} className="flex-shrink-0 w-80 h-[620px] flex flex-col justify-between relative z-10">
                    ...
                    {/* NODE TENGAH: Titik Node Timeline */}
                    <div className="h-12 flex items-center justify-center relative">
                      <div className="w-10 h-10 rounded-full bg-black border-4 border-white shadow-xl flex items-center justify-center z-20 group cursor-pointer hover:scale-125 transition-transform duration-300">
                        <div className="w-3 h-3 rounded-full bg-[#FFD700] group-hover:bg-[#E31837] transition-colors" />
                      </div>
                    </div>
          ...
          {/* 2. MODE MOBILE: Vertical Timeline (block on mobile, hidden on lg) */}
          <div className="block lg:hidden relative pl-6 sm:pl-10 space-y-12 py-4">
            {/* Garis Vertikal Utama di Sisi Kiri */}
            <div className="absolute left-[11px] sm:left-[19px] top-0 bottom-0 w-1 bg-gradient-to-b from-gray-200 via-black to-gray-200" />
            ...
            {filteredEvents.map((event, index) => {
              ...
              return (
                <div key={event.id} className="relative pl-6 sm:pl-8">
                  {/* Titik Node Timeline */}
                  <div className="absolute -left-[27px] sm:-left-[35px] top-4 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-black border-4 border-white shadow-md flex items-center justify-center z-20">
